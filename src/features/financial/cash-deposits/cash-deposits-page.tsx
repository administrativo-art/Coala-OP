...
export function CashDepositsPage() {
  ...
  async function issueSelected() {
    ...
    try {
      ...
      toast({ title: "Cobrança enviada ao Inter", description: "A situação foi confirmada por consulta ativa." });
      setSelected(null);
    } catch (error) {
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-end justify-between gap-3">
      <div>
        <p className="text-xs font-semibold uppercase tracking-wide text-emerald-700">Controle de caixa</p>
        <h1 className="text-2xl font-bold tracking-tight">Depósitos</h1>
        <p className="text-sm text-muted-foreground">Blocos formados por ordem de aprovação. Nenhuma cobrança é emitida automaticamente.</p>
      </div>
    ...
    {!inter.ready && <div className="inline-flex max-w-full items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-2.5 py-1.5 text-xs text-amber-950">
      <Info className="h-3.5 w-3.5 shrink-0" />
      <span className="truncate">
        <strong>Inter em configuração.</strong>{" "}
        A emissão de cobranças ficará disponível após configurar a integração.
      </span>
    </div>}
    ...
    {loading ? <div className="flex h-56 items-center justify-center"><Loader2 className="h-6 w-6 animate-spin" /></div>
      : groups.length === 0 ? <Card><CardContent className="p-10 text-center text-sm text-muted-foreground">Nenhum bloco foi formado. Eles aparecerão após a aprovação de fechamentos com dinheiro contado.</CardContent></Card>
        : <div className="space-y-6">{groups.map(([kioskId, items]) => <section key={kioskId} className="space-y-3">
          <div><h2 className="font-bold">{items[0].kioskName}</h2><p className="text-xs text-muted-foreground">{kioskId}</p></div>
          <div className="grid gap-3 lg:grid-cols-2">{items.map((batch) => {
            ...
            return <Card key={batch.id} id={`deposit-${batch.id}`} className="scroll-mt-24 target:ring-2 target:ring-emerald-500 target:ring-offset-2">
              <CardHeader className="pb-3"><div className="flex items-start justify-between gap-3"><div><CardTitle className="flex items-center gap-2 text-base"><Banknote className="h-4 w-4" />Bloco #{batch.sequence}</CardTitle><p className="mt-1 font-mono text-xs font-semibold text-emerald-700">{reference}</p><p className="mt-1 text-xs text-muted-foreground">{batch.periodStartDate.split("-").reverse().join("/")} a {batch.periodEndDate.split("-").reverse().join("/")} · {batch.dates.length} dia(s)</p></div><Badge variant="outline">{STATUS_LABEL[batch.status]}</Badge></div></CardHeader>
              <CardContent className="space-y-3">
                <div><div className="flex justify-between text-sm"><strong>{formatBRL(batch.totalCents)}</strong><span className="text-muted-foreground">de {formatBRL(batch.maxCents)}</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full bg-emerald-500" style={{ width: `${Math.min(100, (batch.totalCents / batch.maxCents) * 100)}%` }} /></div><p className="mt-1 text-xs text-muted-foreground">Ainda cabem {formatBRL(batch.remainingCapacityCents)}</p></div>
                ...
                {cobranca && <div className="rounded-lg border bg-slate-50 p-3 text-xs"><div className="flex flex-wrap justify-between gap-3"><span>Inter: <strong>{cobranca.situacao ?? cobranca.status}</strong></span><span>Referência bancária: <strong className="font-mono">{cobranca.seuNumero}</strong></span></div>{cobranca.codigoSolicitacao && <p className="mt-2 text-muted-foreground">Código da solicitação: <span className="font-mono">{cobranca.codigoSolicitacao}</span></p>}{cobranca.recebidoManual && <p className="mt-2 text-amber-800">Marcado recebido manualmente; não tratado como liquidação bancária.</p>}{batch.ledgerTransactionId && <p className="mt-2 text-emerald-700">Entrada bancária registrada no financeiro.</p>}{batch.bankWarning && <p className="mt-2 text-amber-800">{batch.bankWarning}</p>}</div>}
                <div className="rounded-lg border">
                  <Button
                    type="button"
                    variant="ghost"
                    className="h-auto w-full justify-between rounded-lg px-3 py-2 text-sm"
                  ...
                  {expanded && <div className="space-y-2 border-t p-3">
                    {dayComposition.length === 0
                      ...
                      : dayComposition.map((day) => {
                        ...
                        return <div key={day.date} className="grid gap-2 rounded-lg bg-slate-50 p-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
                          <div>
                ...
                <div className="flex flex-wrap justify-end gap-2">
                  ...
                  {canIssue && <Button size="sm" variant="outline" onClick={() => setSelected(batch)} disabled={!permissions.financial.cashDeposits.issue || !inter.ready}><FileWarning className="mr-2 h-4 w-4" />{["failed", "cancelled"].includes(batch.status) ? "Reemitir" : "Preparar emissão"}</Button>}
                </div>
              </CardContent>
            </Card>;
          })}</div>
    ...
    <Dialog open={cancelBatch !== null} onOpenChange={(open) => { if (!open) { setCancelBatch(null); setCancelReason(""); } }}>
      <DialogContent>
        <DialogHeader><DialogTitle>Cancelar cobrança Inter</DialogTitle><DialogDescription>O motivo é obrigatório e será enviado ao banco. Se a cobrança já tiver sido paga, a consulta ativa prevalece e o bloco será marcado como pago.</DialogDescription></DialogHeader>
        ...
        <DialogFooter><Button variant="outline" onClick={() => setCancelBatch(null)}>Voltar</Button><Button variant="destructive" onClick={() => void cancelSelected()} disabled={!cancelReason.trim() || submitting}>{submitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Cancelar cobrança</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  </div>;
}
