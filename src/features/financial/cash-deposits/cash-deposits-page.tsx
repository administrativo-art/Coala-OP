    ...
    <Dialog open={selected !== null} onOpenChange={(open) => { if (!open) setSelected(null); }}>
      <DialogContent className="gap-0 overflow-hidden rounded-[20px] p-0 sm:max-w-[540px]">
        ...
        <div className="space-y-5 px-[26px] py-[22px]">
          ...
          <div className="grid gap-3 rounded-xl border border-stone-200 bg-stone-50 p-4 sm:grid-cols-3">
        ...
        <DialogFooter className="border-t border-stone-100 px-[26px] py-4"><Button variant="outline" className="rounded-[10px]" onClick={() => setSelected(null)}>Cancelar</Button><Button className="rounded-[10px] bg-pink-600 font-bold text-white hover:bg-pink-700" onClick={() => void issueSelected()} disabled={((selected?.grossTotalCents ?? 0) !== (coinHoldCents ?? 0) && !inter.ready) || submitting || (coinHoldCents ?? 0) > (selected?.grossTotalCents ?? 0) || (dueChoice === "custom" && !customDueDate)}>{submitting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : (coinHoldCents ?? 0) === (selected?.grossTotalCents ?? 0) ? <Coins className="mr-2 h-4 w-4" /> : <Barcode className="mr-2 h-4 w-4" />}{(coinHoldCents ?? 0) === (selected?.grossTotalCents ?? 0) ? "Separar moedas" : "Emitir boleto"}</Button></DialogFooter>
      </DialogContent>
    </Dialog>
