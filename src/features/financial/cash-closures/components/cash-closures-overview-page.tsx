    ...
    {loading ? <div className="flex h-56 items-center justify-center"><Loader2 className="h-6 w-6 animate-spin" /></div> : units.length === 0 ? <Card className="rounded-2xl border-stone-200"><CardContent className="p-10 text-center text-sm text-muted-foreground">Nenhuma unidade disponível para seu perfil.</CardContent></Card> : <div className="grid gap-3.5 md:grid-cols-2 xl:grid-cols-4">{units.map((unit) => {
      ...
      return <Card key={unit.id} className={`overflow-hidden rounded-[18px] border-stone-200 bg-white shadow-[0_2px_10px_rgba(15,23,42,.05)] ${danger ? "border-l-4 border-l-rose-500" : pending ? "border-l-4 border-l-amber-500" : summary ? "border-l-4 border-l-emerald-500" : "border-l-4 border-l-stone-300"}`}>
        ...
        <CardContent className="space-y-2.5 px-3.5 pb-3.5">
          <div className="grid grid-cols-4 gap-1.5 text-center"><div className="rounded-xl bg-amber-50 px-1 py-2"><p className="text-[17px] font-black leading-none text-amber-800">{summary?.pendingCount ?? 0}</p><p className="mt-1 truncate text-[9.5px] font-bold text-amber-700">Pendentes</p></div><div className="rounded-xl bg-rose-50 px-1 py-2"><p className="text-[17px] font-black leading-none text-rose-800">{summary?.divergentCount ?? 0}</p><p className="mt-1 truncate text-[9.5px] font-bold text-rose-700">Divergentes</p></div><div className="rounded-xl bg-emerald-50 px-1 py-2"><p className="text-[17px] font-black leading-none text-emerald-800">{summary?.approvedCount ?? 0}</p><p className="mt-1 truncate text-[9.5px] font-bold text-emerald-700">Aprovados</p></div><div className="rounded-xl bg-stone-100 px-1 py-2"><p className="text-[17px] font-black leading-none text-zinc-600">{summary?.syncErrorCount ?? 0}</p><p className="mt-1 truncate text-[9.5px] font-bold text-zinc-400">Erros</p></div></div>
          ...
          <div className="space-y-1 text-[10.5px] font-semibold text-zinc-400">
            ...
            <div className="flex items-center justify-between gap-2"><span>Aprovado:</span><span>{summary?.lastApprovedDate?.split("-").reverse().join("/") ?? "—"}</span></div>
          </div>
