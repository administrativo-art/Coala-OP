...
export function CashClosureCalendarPage({ kioskId, year, month }: { kioskId: string; year: number; month: number }) {
  ...
  return <div className="w-full max-w-none space-y-4">
    ...
    })}</div><div className="mt-3.5 flex flex-wrap items-center gap-x-3.5 gap-y-2 border-t border-stone-100 pt-3 text-[11px] font-semibold text-zinc-500"><Legend color="border-emerald-200 bg-emerald-50" label="Bateu" /><Legend color="border-rose-200 bg-rose-50" label="Divergência" /><Legend color="border-amber-200 bg-amber-50" label="Pendente" /><span className="hidden h-3.5 w-px bg-stone-200 sm:block" /><span className="flex items-center gap-1"><CheckCircle2 className="h-3 w-3 text-emerald-700" />Aprovado</span><span className="flex items-center gap-1"><Clock3 className="h-3 w-3 text-amber-700" />Em revisão</span><span className="flex items-center gap-1"><CircleAlert className="h-3 w-3 text-zinc-500" />Rascunho</span></div></CardContent></Card>}
  </div>;
}
