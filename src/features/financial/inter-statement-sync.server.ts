...
type SyncResult = {
  ...
  matchingCoverage: {
    expenseCandidatesComplete: boolean;
...
function resolvedLedgerCandidate(
  transactionId: string,
  data: Record<string, unknown>,
): ExistingInterLedgerCandidate | null {
  const expenseId = String(data.linkedExpenseId || data.expenseId || "").trim();
  const date = asDate(data.date || data.paidAt || data.createdAt);
  const amount = Number(data.amount) || 0;
  if (data.auditStatus !== "resolved" || !expenseId || !date || amount <= 0) return null;
  return {
    transactionId,
    direction: data.direction === "in" || data.type === "transfer_in" ? "in" : "out",
    amount,
    date,
    ...
    references: Array.isArray(data.bankReferences)
...
function buildSummary(items: Array<Record<string, unknown>>) {
  return {
    ...
    audited: items.filter((item) => String(item.status || "") === "audited").length,
    ignored: items.filter((item) => String(item.status || "") === "ignored").length,
...
function buildSessionItem(params: {
  ...
  const prelinkedSuggestion = suggestion?.confidence === "high" ? suggestion : null;
  const linkedExpenseId = match?.expenseId || prelinkedSuggestion?.expenseId || existingLedger?.expenseId || "";
  ...
  const suggestedPrincipal = suggestedExpense?.settlementPrincipalValue || suggestedExpense?.value;
  const suggestedInterest = suggestedExpense?.interest || (chargesMatchBankBreakdown ? bankCharges.interest : 0);
  const suggestedFine = suggestedExpense?.fine || (chargesMatchBankBreakdown ? bankCharges.fine : 0);
  ...
  return {
    ...
    suggestedInstallmentValue: suggestedPrincipal || (existingLedger ? Math.abs(entry.amount) : null),
    suggestedAdditionalCharges: additionalCharges || null,
    suggestedReportedPaymentId: suggestedExpense?.reportedPaymentId || null,
    suggestedReportedLinkId: suggestedExpense?.reportedLinkId || null,
    suggestedConfidence: suggestion?.confidence || (match || existingLedger ? "high" : null),
    expenseDraft: {
      mode: linkedExpenseId ? "existing" : "new",
      linkedExpenseId,
      reportedPaymentId: suggestedExpense?.reportedPaymentId || "",
      reportedLinkId: suggestedExpense?.reportedLinkId || "",
      purchaseOrderId: "",
      purchaseLinkMode: "goods",
      allocatedAmount: Math.abs(entry.amount),
      settlementBaseValue: suggestedPrincipal || Math.abs(entry.amount),
      settlementInstallmentNumber: suggestedExpense?.installmentNumber || 0,
      interest: suggestedInterest,
      fine: suggestedFine,
      discount: suggestedExpense?.discount || 0,
      abatement: suggestedExpense?.abatement || 0,
      chargesAccountPlanId: suggestedExpense?.chargesAccountPlanId || "",
      chargesAccountPlanName: suggestedExpense?.chargesAccountPlanName || "",
      description,
      supplier: alias?.supplier || suggestedExpense?.supplier || "",
    ...
    status: existingLedger || !isDebit ? "completed" : "pending",
  };
}
...
async function loadPendingExpenseMatches() {
  const [pendingSnapshot, partialSnapshot, reportedPaymentsSnapshot] = await Promise.all([
    ...
    financialDbAdmin.collection("expenses").where("status", "==", "partially_paid").limit(MATCH_QUERY_LIMITS.partiallyPaidExpenses).get(),
    financialDbAdmin.collection("payments").where("status", "==", "REPORTED").limit(MATCH_QUERY_LIMITS.reportedPayments).get(),
  ]);
  const complete =
    pendingSnapshot.size < MATCH_QUERY_LIMITS.pendingExpenses &&
    partialSnapshot.size < MATCH_QUERY_LIMITS.partiallyPaidExpenses &&
    reportedPaymentsSnapshot.size < MATCH_QUERY_LIMITS.reportedPayments;
  const expenseDocuments = [...pendingSnapshot.docs, ...partialSnapshot.docs];
  const expenseCandidates = expenseDocuments.flatMap((document): PendingExpenseMatch[] => {
    ...
    if (expense.status === "partially_paid" && outstandingBalance > 0) {
      const dueDate = asDate(expense.dueDate);
      if (!dueDate) return [];
      return [{
        candidateKey: `balance:${document.id}`,
    ...
    if (installments.length > 0) {
      return installments.flatMap((installment: Record<string, unknown>, index: number) => {
        ...
        if (!dueDate) return [];
        return [{
          candidateKey: `installment:${document.id}:${Number(installment.number) || index + 1}`,
          expenseId: document.id,
          expenseDescription: String(expense.description || "Despesa"),
          supplier: String(expense.supplier || ""),
    ...
    return [{
      candidateKey: `expense:${document.id}`,
      expenseId: document.id,
      expenseDescription: String(expense.description || "Despesa"),
      supplier: String(expense.supplier || ""),
  ...
  const reportedPaymentCandidates = reportedPaymentsSnapshot.docs.flatMap((document): PendingExpenseMatch[] => {
    const payment = document.data();
    const expenseId = String(payment.expenseId || "");
    const paidAt = asDate(payment.paidAt);
    const cashValue = Number.isInteger(payment.cashAmountCents)
      ? Number(payment.cashAmountCents) / 100
      : Number(payment.totalPaid) || 0;
    const principalValue = Number.isInteger(payment.principalAmountCents)
      ? Number(payment.principalAmountCents) / 100
      : Number(payment.baseValue) || cashValue;
    if (!expenseId || !paidAt || cashValue <= 0 || principalValue <= 0) return [];
    return [{
      candidateKey: `reported:${document.id}`,
      expenseId,
      expenseDescription: String(payment.description || "Pagamento informado"),
      supplier: String(payment.supplier || ""),
      dueDate: paidAt,
      value: cashValue,
      settlementPrincipalValue: principalValue,
      reportedPaymentId: document.id,
      reportedLinkId: String(payment.linkId || "") || undefined,
      interest: Number(payment.interest) || 0,
      fine: Number(payment.fine) || 0,
  ...
  return {
    candidates: complete ? [...reportedPaymentCandidates, ...expenseCandidates] : [],
    complete,
    documentsRead: pendingSnapshot.size + partialSnapshot.size + reportedPaymentsSnapshot.size,
  };
}

async function loadAliases() {
  const snapshot = await financialDbAdmin.collection("importAliases").limit(IMPORT_ALIAS_LIMIT).get();
...
async function loadExistingInterLedgerCandidates() {
  const [paymentTransactions, depositTransactions] = await Promise.all([
    financialDbAdmin.collection("transactions").where("createdBy", "==", "bank-reconciliation").limit(MATCH_QUERY_LIMITS.existingLedgerPerSource).get(),
    financialDbAdmin.collection("transactions").where("createdBy", "==", "inter-reconciliation").limit(MATCH_QUERY_LIMITS.existingLedgerPerSource).get(),
  ]);
  const complete =
    paymentTransactions.size < MATCH_QUERY_LIMITS.existingLedgerPerSource &&
    depositTransactions.size < MATCH_QUERY_LIMITS.existingLedgerPerSource;
  const documents = new Map(
    [...paymentTransactions.docs, ...depositTransactions.docs].map((document) => [document.id, document])
  );

  const candidates = [...documents.values()].flatMap((document): ExistingInterLedgerCandidate[] => {
  ...
  return {
    ...
    complete,
    documentsRead: paymentTransactions.size + depositTransactions.size,
  };
}
...
async function registerEntry(params: {
  ...
  const { entry, accountId, accountName, alias, existingLedger } = params;
  ...
  return financialDbAdmin.runTransaction(async (transaction) => {
    ...
    if (existingEvent.exists) {
      ...
      const linkedTransactionRef = financialDbAdmin.collection("transactions").doc(linkedTransactionId);
      const linkedTransaction = await transaction.get(linkedTransactionRef);
      const appliedLedger = linkedTransaction.exists
        ? resolvedLedgerCandidate(linkedTransactionId, linkedTransaction.data() || {})
        : null;
      transaction.set(
    ...
    if (existingTransaction.exists) {
      const appliedLedger = resolvedLedgerCandidate(transactionId, existingTransaction.data() || {});
      transaction.create(eventRef, {
        provider: "inter",
        accountId,
        externalTransactionId: entry.externalId,
      ...
      return {
        inserted: false,
        matched: Boolean(existingTransaction.get("autoMatched")),
    ...
    const resolvedDescription = alias?.descriptionOverride || entry.description;
    transaction.create(transactionRef, {
      ...
      rawBankDescription: entry.description,
      ...statementMetadata(entry),
      auditStatus: entry.amount >= 0 ? "resolved" : "pending",
      autoMatched: false,
    ...
    transaction.create(eventRef, {
      ...
      linkedTransactionId: transactionId,
      linkedExpenseId: null,
      reconciliationMode: "statement_entry",
      createdAt: Timestamp.now(),
      createdBy: SYSTEM_ACTOR,
    });

    return { inserted: true, matched: false, reconciledExisting: false, transactionId };
  });
}
...
async function updateMonthlySession(params: {
  ...
  await financialDbAdmin.runTransaction(async (transaction) => {
    ...
    const refreshedCurrentItems = currentItems.map((item: Record<string, unknown>) => {
      ...
      return refreshStatementSessionItem(item, incoming);
    });
...
export async function syncInterStatement(): Promise<SyncResult> {
  ...
  await stateRef.set({
    ...
    matchingCoverage: {
      expenseCandidatesComplete: candidateLoad.complete,
