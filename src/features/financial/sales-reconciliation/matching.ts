...
function scopedKey(fact: SalesMatchFact, value: string | null) {
  return value
    ? `${fact.workspaceId}:${fact.businessDate}:${fact.channel}:${value}`
    : null;
}
...
function authorizationKey(fact: SalesMatchFact) {
  ...
  return nsu && authorization && terminal
    ? scopedKey(fact, `${nsu}:${authorization}:${terminal}`)
    : null;
}
...
function merchantOrderKey(fact: SalesMatchFact) {
  return scopedKey(
    fact,
    normalizedId(fact.source === "pdv" ? fact.couponId : fact.identifiers.merchantOrderId),
  );
}
...
function deterministicKey(pdv: SalesMatchFact[], stone: SalesMatchFact[]) {
  return `pdv=${pdv.map((fact) => fact.id).sort().join(",")}|stone=${stone.map((fact) => fact.id).sort().join(",")}`;
}
...
function buildCase(
  ...
  const hasUnmappedStone = stone.some((fact) => !fact.kioskId);
  let kind: SuggestedSalesReconciliationCase["kind"];
  if (forceAmbiguous) kind = "ambiguous";
  else if (hasUnmappedStone) kind = "unit_unmapped";
  ...
  return {
    ...
    matchBasis: basis,
    confidence,
    reviewStatus: kind === "matched" && confidence === "high" ? "matched_auto" : "pending_review",
  };
}
...
export function suggestSalesReconciliationCases(input: {
  ...
  function matchByKey(
    ...
    for (const key of [...pdvGroups.keys()].sort()) {
      ...
      cases.push(buildCase(pdv, stone, basis, "high"));
  ...
  while (matchedUnique) {
    ...
    for (const pdvId of [...unmatchedPdv].sort()) {
      ...
      cases.push(buildCase([pdvById.get(pdvId)!], [stoneById.get(stoneId)!], "unique_amount_time", "high"));
  ...
  for (const startPdvId of [...unmatchedPdv].sort()) {
    ...
    while (queue.length > 0) {
      ...
      if (current.source === "pdv") {
        ...
        for (const stoneId of unmatchedStone) {
          if (candidateEdge(pdv, stoneById.get(stoneId)!, timeWindowMs)) queue.push({ source: "stone", id: stoneId });
        }
      } else {
        ...
        for (const pdvId of unmatchedPdv) {
          if (candidateEdge(pdvById.get(pdvId)!, stone, timeWindowMs)) queue.push({ source: "pdv", id: pdvId });
        }
      }
    }
