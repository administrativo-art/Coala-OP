...
import { ArrowRight, Check, Clock, GraduationCap, Milestone, ShieldCheck, Users } from "lucide-react";
...
const EXPERIENCE_STAGES: NavigationStage[] = [
  { id: "probation", label: "Experiência e treinamento", short: "Experiência", owner: "Liderança", focus: "Trilhas, avaliações dos primeiros 90 dias e decisão de efetivação." },
  { id: "done", label: "Finalizado", short: "Finalizado", owner: "RH", focus: "Processo encerrado e histórico consolidado para auditoria." },
];
...
function normalizedPhase(value?: PhaseId | null): PhaseId {
  ...
  if (value === "training") return "probation";
...
export function onboardingConceptualStageNumber(
  process: Pick<OnboardingProcess, "currentStage" | "status">,
) {
  if (process.status === "completed" || process.currentStage === "done") return 7;
  const phase = normalizedPhase(process.currentStage);
  if (phase === "accountant") return 2;
  if (phase === "signature_preparation") return 3;
  if (phase === "formalization_validation") return 4;
  if (phase === "integration") return 5;
  if (phase === "probation") return 6;
  return 1;
}
...
export function OnboardingDetailNavigation({ process, activePhaseId, onSelect }: DetailNavigationProps) {
  ...
  const formalizationComplete = Boolean(
    process.formalizationCompletedAt
    || process.completedAt
  ...
  const selectedTarget = actualTarget(process, selectedStage.id);
  const selectedProcessStage = (process.stages ?? []).find((stage) => stage.id === selectedTarget)
    ?? (process.stages ?? []).find((stage) => normalizedPhase(stage.id) === selectedStage.id);
  const selectedDueDays = selectedProcessStage?.dueDays ?? null;
  const selectedIndex = FORMALIZATION_STAGES.findIndex((stage) => stage.id === active);
  const selectedExperienceIndex = EXPERIENCE_STAGES.findIndex((stage) => stage.id === active);
  const selectedIsExperience = EXPERIENCE_STAGES.some((stage) => stage.id === active);
  const selectedState = selectedIsExperience
    ? active === "done"
      ? process.status === "completed" || process.currentStage === "done" ? "Concluída" : "Etapa futura"
      : formalizationComplete ? "Disponível" : "Prévia"
  ...
  const renderTab = (stage: NavigationStage, index: number, experience = false) => {
    ...
    const isFinalStage = stage.id === "done";
    const availableExperience = experience && stage.id === "probation" && formalizationComplete;
    const doneExperience = experience && isFinalStage && (process.status === "completed" || process.currentStage === "done");
    const tabDone = done || doneExperience;
    const tabCurrent = currentStage || doneExperience;
    const stateLabel = tabDone
      ? "Concluída"
      : tabCurrent
        ? "Etapa atual"
        : availableExperience
    ...
    return (
      ...
      >
        <span className="flex items-center gap-2">
          ...
          <span className={`h-1.5 w-1.5 rounded-full ${tabDone || availableExperience ? "bg-emerald-500" : tabCurrent || selected ? "bg-[#df2f78]" : "bg-stone-300"}`} />
        </span>
    ...
    <div className="space-y-3">
      <div className="flex items-stretch gap-2 overflow-x-auto pb-1">
        {formalizationOpen ? (
          ...
          <button type="button" onClick={() => onSelect("integration")} className="flex w-[90px] shrink-0 flex-col items-center justify-center gap-2.5 rounded-2xl border border-[#e2ded6] bg-[#faf9f6] px-2 py-4 text-stone-600 hover:bg-white">
        ...
        <div className={`flex w-[74px] shrink-0 flex-col items-center justify-center gap-2 rounded-[18px] border border-dashed px-2 text-center ${formalizationComplete ? "border-emerald-300 bg-emerald-50 text-emerald-700" : "border-stone-300 bg-[#faf9f6] text-stone-400"}`}>
          <span className={`grid h-6 w-6 place-items-center rounded-full ${formalizationComplete ? "bg-emerald-600 text-white" : "bg-stone-200 text-stone-500"}`}><Milestone className="h-3.5 w-3.5" /></span>
        ...
        {!formalizationOpen ? (
          <section className="min-w-[360px] flex-1 rounded-[18px] border border-violet-200 bg-white p-4 shadow-sm">
            ...
            <div className="flex gap-1">{EXPERIENCE_STAGES.map((stage, index) => renderTab(stage, FORMALIZATION_STAGES.length + index, true))}</div>
          </section>
        ) : (
          <button type="button" onClick={() => onSelect("probation")} className="flex w-[90px] shrink-0 flex-col items-center justify-center gap-2.5 rounded-2xl border border-violet-200 bg-violet-50/40 px-2 py-4 text-violet-700 hover:bg-violet-50">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.09em] [writing-mode:vertical-rl] [transform:rotate(180deg)]">Treinamento</span>
            <span className="font-mono text-[10px] font-bold">0/2 etapas</span>
          </button>
        )}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-[#e2ded6] bg-white px-4 py-3 shadow-sm">
        <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-slate-950 font-mono text-sm font-bold text-white">
          {selectedIsExperience ? FORMALIZATION_STAGES.length + selectedExperienceIndex + 1 : selectedIndex + 1}
        </span>
        <span className="min-w-0 flex-1">
          <span className="flex flex-wrap items-center gap-2">
            <span className="text-[15px] font-black tracking-[-0.02em]">{selectedStage.label}</span>
            <span className={`rounded-lg px-2.5 py-1 text-[9.5px] font-black uppercase tracking-[0.05em] ${isPreview ? "bg-amber-50 text-amber-700" : selectedState === "Concluída" ? "bg-emerald-50 text-emerald-700" : "bg-pink-50 text-pink-700"}`}>{selectedState}</span>
            <span className="inline-flex items-center gap-1 rounded-lg border border-stone-200 bg-stone-50 px-2.5 py-1 text-[9.5px] font-black uppercase tracking-[0.05em] text-stone-500">
              <Users className="h-3.5 w-3.5" />
              {selectedStage.owner}
            </span>
            {selectedDueDays ? (
              <span className="inline-flex items-center gap-1 rounded-lg border border-amber-200 bg-amber-50 px-2.5 py-1 text-[9.5px] font-black uppercase tracking-[0.05em] text-amber-700">
                <Clock className="h-3.5 w-3.5" />
                Prazo {selectedDueDays} dia{selectedDueDays === 1 ? "" : "s"}
              </span>
            ) : null}
          </span>
          <span className="mt-1 block text-xs font-medium text-stone-600">{selectedStage.focus}</span>
        </span>
        {isPreview ? (
