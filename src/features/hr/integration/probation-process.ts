...
export const DEFAULT_PROBATION_RUNTIME_CONFIG: ProbationRuntimeConfig = { ...DEFAULT_PROBATION_CONFIG, alertsBeforeDays: [10, 5, 1], extensionTermRequired: true, evaluator: { strategy: "direct_manager", fallback: "hr_pool" }, effectivenessRule: "manual" };
...
export function createProbationProcess(admissionDate: string | null, input: Partial<ProbationRuntimeConfig> | undefined, now: string): ProbationProcessState {
  const config: ProbationRuntimeConfig = { ...DEFAULT_PROBATION_RUNTIME_CONFIG, ...input, evaluator: { ...DEFAULT_PROBATION_RUNTIME_CONFIG.evaluator, ...(input?.evaluator ?? {}) }, alertsBeforeDays: Array.from(new Set(input?.alertsBeforeDays ?? DEFAULT_PROBATION_RUNTIME_CONFIG.alertsBeforeDays)).sort((a, b) => b - a) };
...
export function refreshProbationProcess(state: ProbationProcessState, today: string): ProbationProcessState {
  ...
  const evaluations = state.evaluations.map((evaluation) => evaluation.status === "completed" ? evaluation : { ...evaluation, status: today < evaluation.windowStartDate ? "scheduled" as const : today <= evaluation.windowEndDate ? "available" as const : "overdue" as const });
  const alerts = state.alerts.map((alert) => alert.status === "sent" ? alert : { ...alert, status: today >= alert.dueDate ? "due" as const : "scheduled" as const });
  const allCompleted = evaluations.length > 0 && evaluations.every((evaluation) => evaluation.status === "completed");
  let status: ProbationProcessState["status"] = state.status;
  if (allCompleted || today > state.schedule.finalEndDate) status = "decision_pending";
  if (state.config.effectivenessRule === "approved_evaluations" && allCompleted && evaluations.every((evaluation) => evaluation.result === "approved")) status = "effective";
  if (state.config.effectivenessRule === "automatic_at_end" && today > state.schedule.finalEndDate) status = "effective";
...
export function decideProbation(state: ProbationProcessState, params: { result: "effective" | "terminated"; notes?: string; actorId: string; now: string }) {
  if (state.extensionTerm.required && state.evaluations.some((evaluation) => evaluation.id === "second") && !state.extensionTerm.signedAt) throw new Error("O termo de prorrogação obrigatório ainda não foi assinado.");
