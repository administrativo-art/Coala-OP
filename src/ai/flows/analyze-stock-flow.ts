...
const analyzeStockPrompt = ai.definePrompt({
  ...
  prompt: `
    ...
    Your task is to analyze a stock report provided as a PDF, line by line, and list the current stock and any replenishment needs for **every item found in the report**.
    ...
    2.  **Process ALL PDF Products:** For **each product** listed in the PDF, find its corresponding entry in the \`products\` configuration list. You MUST process every single item from the PDF.
    ...
    4.  **Calculate Needs for PDF Products:** For every product found in the PDF, calculate the replenishment need. A product needs replenishment **only if** its \`currentStockInBaseUnit\` is **BELOW** the configured \`min\` stock level for that kiosk.
    ...
    5.  **Include ALL Items in Results:** CRITICAL: The final \`results\` array in your JSON output MUST contain an entry for **every single product found in the PDF report**, regardless of whether its \`neededInBaseUnit\` is greater than 0 or not. Do NOT filter out any items.
    6.  **Summarize:** Create a concise \`summary\` based on the results. For example, "Analisados X itens do relatório, dos quais Y precisam de reposição.". If no items need replenishment, state that.
