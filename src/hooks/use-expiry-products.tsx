  ...
  const adjustLotQuantity = useCallback(async (
    ...
    await runTransaction(db, async (transaction) => {
      for (const item of session.items) {
        ...
        if (item.adjustment && item.adjustment.quantity > 0) {
        ...
        // 2. Process divergences
        if (item.divergences && item.divergences.length > 0) {
          for (const divergence of item.divergences) {
            accumulatedChange -= divergence.quantity;
        ...
        // 3. Apply accumulated changes to the lot
        if (accumulatedChange !== 0) {
            transaction.update(lotRef, { quantity: increment(accumulatedChange) });
        }
      }
    });
  }, []);
