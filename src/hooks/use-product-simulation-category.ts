...
export const useProductSimulationCategory = (): ProductSimulationCategoryContextType => {
  const context = useContext(ProductSimulationCategoryContext);
  if (context === undefined) {
    throw new Error('useProductSimulationCategory must be used within a ProductSimulationCategoryProvider');
  }
  return context;
};
