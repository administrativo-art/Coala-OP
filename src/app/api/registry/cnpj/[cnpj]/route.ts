...
import { canLookupCompany, getCompanyUserContext } from '@/app/api/companies/_auth';
import { CompanyNormalizer } from '@/lib/company/company-normalizer';
...
export async function GET(request: NextRequest, context: { params: Promise<{ cnpj: string }> }) {
  ...
  return NextResponse.json({
    ...result,
    entity: result.entity ?? (result.company ? CompanyNormalizer.toEntity(result.company) : undefined),
    sources: result.sources.map((source) => source.fonte),
