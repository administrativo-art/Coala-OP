...
import { employeeCodeFrom, loadExpectedIdentity } from "@/lib/hr/employee-document-identity";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const mimeType = String(metadata.contentType ?? item.mimeType ?? "application/octet-stream");
    ...
    const match = identity
      ? matchEmployeeAgainstExpected({
          extracted: {
            cpf: asString(fields.cpf), registrationNumber: asString(fields.registrationNumber),
            name: ai.identifiedEmployeeName ?? asString(fields.employeeName),
            birthDate: asString(fields.birthDate), admissionDate: asString(fields.admissionDate),
          },
          expected: identity,
        })
    ...
    const decision = decideDocumentAction({
      ...
      employeeMatchStatus: match.status, multipleDocumentsDetected: ai.structure.multipleDocumentsDetected,
    ...
    await itemRef.update({
      ...
      confidence: ai.documentTypeConfidence, employeeMatchStatus: match.status, employeeMatchReason: match.reason,
      aiEmployeeMatchStatus: ai.employeeMatchStatus, identifiedEmployeeName: ai.identifiedEmployeeName ?? null,
      extractedFields: fields, fieldConfidences: ai.fieldConfidences, structure: ai.structure, destination,
    ...
    await itemRef.collection("audit").add({
      ...
      documentTypeCode: config.code, employeeMatchStatus: match.status, decision: decision.action,
    ...
    return NextResponse.json({
      ...
      documentType: config.label, category: config.category, employeeMatchStatus: match.status,
      employeeMatchReason: match.reason, decisionReason: decision.reason, destinationTrail: destination.pathSegments,
      fileName: destination.downloadName, fieldConfidences: ai.fieldConfidences, extractedFields: fields,
      warnings: ai.warnings, errors: ai.issues, analysisModel: ai.model,
    });
  } catch (cause) {
