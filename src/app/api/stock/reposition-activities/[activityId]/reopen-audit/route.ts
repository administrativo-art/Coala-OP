...
import { reopenAuditServer } from "@/features/reposition/lib/server";
  ...
  try {
    ...
    if (!(context.isDefaultAdmin || context.permissions.reposition.cancel)) {
      return NextResponse.json(
        { error: "Sem permissão para reabrir auditoria." },
        { status: 403 }
      );
    }
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error ? error.message : "Falha ao reabrir auditoria.",
      },
      { status: 400 }
    );
  }
