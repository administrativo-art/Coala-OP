...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!actor.isDefaultAdmin && !actor.permissions.financial?.paymentRequests?.submit) return NextResponse.json({ error: "Sem permissão para enviar pagamentos ao banco." }, { status: 403 });
