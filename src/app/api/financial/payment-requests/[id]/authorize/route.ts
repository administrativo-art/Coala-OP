...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!actor.isDefaultAdmin && !actor.permissions.financial?.paymentRequests?.authorize) return NextResponse.json({ error: "Sem permissão para autorizar pagamentos." }, { status: 403 });
