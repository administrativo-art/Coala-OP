...
export default function DocumentTemplatesPage() {
  ...
  async function createTemplate() {
    ...
    try {
      ...
      setTemplates((current) => [uploadPayload.template, ...current]);
      setDialogOpen(false);
      setName("");
  ...
  if (!canManage) return <p className="p-6 text-sm text-slate-500">Sem permissão para acessar modelos.</p>;
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <Button size="sm" className="h-9 gap-2 rounded-lg" onClick={() => setDialogOpen(true)}><Plus className="h-4 w-4" />Novo modelo</Button>
      </div>
      ...
      {loading ? <div className="grid min-h-40 place-items-center"><Loader2 className="h-5 w-5 animate-spin text-teal-700" /></div> : filtered.length === 0 ? (
        ...
        <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">
          {filtered.map((template) => (
            <article key={template.id} className="flex min-h-24 items-start gap-3 rounded-lg border bg-white p-3 shadow-sm">
