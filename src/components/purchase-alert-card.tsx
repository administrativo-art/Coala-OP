...
export function PurchaseAlertCard() {
    ...
    return (
        <Card className="hover:bg-muted/50 transition-colors h-full col-span-full border border-border bg-card">
                ...
                {!hasAlerts ? (
                    ...
                    <ScrollArea className="h-[250px] w-full border-none">
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    <TableHead className="pl-4">Insumo</TableHead>
                            ...
                            <TableBody>
                             {projectionResults.map(item => (
                                <TableRow key={item.baseProductId}>
                                    <TableCell className="font-bold pl-4 text-sm">{item.baseProductName}</TableCell>
                                    <TableCell className="text-right">
                                        <div className="flex flex-col items-end">
                                            <span className="font-bold">{item.currentStock.toFixed(0)} {item.baseProductUnit}</span>
                                            <div className="w-16 h-1 mt-1 bg-muted rounded-full overflow-hidden">
                                                <div className="h-full bg-amber-500" style={{ width: `${Math.min((item.currentStock / (item.minimumStock || 1)) * 100, 100)}%` }} />
                                            </div>
                                        </div>
                                    </TableCell>
                                    <TableCell className="text-right font-black text-amber-500">{isFinite(item.daysOfCoverage) ? `${item.daysOfCoverage} dias` : 'N/A'}</TableCell>
                                    <TableCell className="text-center font-bold text-destructive">
                                        {item.orderDate ? format(item.orderDate, 'dd/MM/yy') : 'Vencendo'}
                                    </TableCell>
                                    <TableCell className="text-right">{getStatusBadge(item)}</TableCell>
                                    <TableCell className="text-right pr-4">
                                        <Button variant="outline" size="sm" className="h-8 bg-card hover:bg-accent border-border/50 shadow-sm font-bold">
                                            Pedir agora
                                        </Button>
                                    </TableCell>
                                </TableRow>
...
export function PurchaseAlertSummary() {
    ...
    return (
        <Link href="/dashboard/stock/purchasing" className="group">
            <Card className="hover:bg-muted/50 hover:-translate-y-px transition-all h-full border border-rose-500/50 bg-card rounded-xl">
