...
export interface StockAnalysisContextType {
  ...
  updateReport: (report: StockAnalysisReport) => Promise<void>;
...
export function StockAnalysisProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, async (querySnapshot) => {
      // Seeding logic is removed to avoid conflict with the new complex structure
  ...
  const updateReport = useCallback(async (report: StockAnalysisReport) => {
    const reportRef = doc(db, "stockAnalysisReports", report.id);
    const { id, ...dataToUpdate } = report;
    try {
