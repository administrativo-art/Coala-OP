...
export function ConsumptionComparisonModal({ open, onOpenChange, history, products, kiosks }: ConsumptionComparisonModalProps) {
    ...
    const handleCompare = (values: ComparisonFormValues) => {
        ...
        allProductIds.forEach(productId => {
            ...
            if (consumptionA > 0) {
                variationPercent = (variation / consumptionA) * 100;
            } else if (consumptionA === 0 && consumptionB > 0) {
                variationPercent = Infinity;
            } else {
                variationPercent = 0;
            }
    ...
    const handleAiAnalysis = async () => {
        ...
        const periodALabel = `${String(monthA).padStart(2, '0')}/${yearA}`;
        const periodBLabel = `${String(monthB).padStart(2, '0')}/${yearB}`;
