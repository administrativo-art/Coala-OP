...
export function ItemAdditionRequestManagement() {
  const { requests, loading, updateRequestStatus, deleteRequest } = useItemAddition();
  ...
  return (
    <>
                  ...
                  <AccordionContent className="p-4 pt-0">
                    ...
                    <div className="flex gap-2 mt-4 border-t pt-4">
                        {req.status === 'pending' && (
                         ...
                         <Button size="sm" variant="ghost" className="text-destructive hover:text-destructive" onClick={() => setRequestToDelete(req.id)}>
      ...
      <DeleteConfirmationDialog 
        ...
        itemName={`a solicitação de "${requests.find(r=>r.id === requestToDelete)?.productName}"`}
