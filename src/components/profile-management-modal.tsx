...
import { PlusCircle, Edit, Trash2, ShieldCheck, Package, Box, Warehouse, UserCog, ClipboardList } from 'lucide-react';
...
export function ProfileManagementModal({ open, onOpenChange, canEdit }: ProfileManagementModalProps) {
  ...
  const handleAddNew = () => {
    setEditingProfile(null);
    form.reset({ name: '', permissions: defaultPermissions });
    setShowForm(true);
  };
          ...
          {showForm ? (
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                />
                <Accordion type="multiple" defaultValue={['products', 'lots', 'predefinedLists', 'kiosks', 'users']} className="w-full">
