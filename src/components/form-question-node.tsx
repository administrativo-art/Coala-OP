...
const COLOR_OPTIONS = ['#FEE2E2', '#FEF3C7', '#D1FAE5', '#DBEAFE', '#E0E7FF', '#F3E8FF', '#FCE7F3', '#E5E7EB'];
  ...
  return (
    <>
      ...
      <Card className={cn(
          ...
          selected ? 'ring-2 ring-primary border-primary' : (data.customColor ? '' : visuals.color),
          hasError && '!border-destructive'
      )}
      style={{
          borderColor: selected ? 'hsl(var(--primary))' : (data.customColor || undefined),
          backgroundColor: selected ? undefined : (data.customColor ? `${data.customColor}20` : undefined),
      }}
      >
                    ...
                    <PopoverContent className="w-auto p-2" onClick={(e) => e.stopPropagation()}>
                        <div className="flex gap-1">
                            {COLOR_OPTIONS.map(color => (
                                <button key={color} onClick={() => data.onUpdate({ customColor: color })} className={cn("h-6 w-6 rounded-full border", data.customColor === color && 'ring-2 ring-ring') } style={{ backgroundColor: color }}/>
                            ))}
                             <button onClick={() => data.onUpdate({ customColor: undefined })} className={cn("h-6 w-6 rounded-full border flex items-center justify-center bg-muted", !data.customColor && 'ring-2 ring-ring')}><X className="h-4 w-4 text-muted-foreground"/></button>
                        </div>
                    </PopoverContent>
                </Popover>
