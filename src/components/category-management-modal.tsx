...
export function CategoryManagementModal({ open, onOpenChange }: { open: boolean, onOpenChange: (open: boolean) => void }) {
    ...
    const handleDeleteClick = (category: SimulationCategory) => {
        const isUsed = simulations.some(s => s.categoryIds.includes(category.id));
        if (isUsed) {
        ...
        <>
            <Dialog open={open} onOpenChange={onOpenChange}>
                <DialogContent className="max-w-xl">
                    <DialogHeader>
                        <DialogTitle>Gerenciar Categorias de Simulação</DialogTitle>
                        <DialogDescription>Crie, edite e organize as categorias para suas mercadorias.</DialogDescription>
                    </DialogHeader>
