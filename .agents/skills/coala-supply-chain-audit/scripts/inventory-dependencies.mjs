  ...
  for (const manifestPath of manifestPaths) {
    ...
    for (const group of dependencyGroups) {
      for (const [name, spec] of Object.entries(manifest[group] ?? {})) {
        const entry = { name, spec: String(spec), group, classification: classifyVersionSpec(spec) };
        dependencies.push(entry);
        const locations = packageLocations.get(name) ?? [];
        locations.push({ manifest: relative(root, manifestPath), spec: String(spec), group });
