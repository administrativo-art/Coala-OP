...
import { sanitizeError, sanitizeMetadata } from "../../../../src/lib/observability/sanitize.ts";
...
export function normalizeTriageEvents(records) {
  return records.map((original, index) => {
    ...
    const stack = sanitizeError(record.stackSanitized ?? record.stack ?? "").message || undefined;
    ...
    return {
      ...
      incomplete: !record.errorCode || !record.source || !record.occurredAt,
    };
  });
}
...
export function runErrorTriage({ repositoryRoot, inputPath, issuesPath, outputDirectory, now = new Date(), dryRun = false }) {
  ...
  if (!existsSync(input)) throw new Error("Arquivo de entrada não encontrado.");
  const records = parseJsonOrJsonl(readFileSync(input, "utf8"));
  const normalized = normalizeTriageEvents(records);
  const issues = issuesPath ? JSON.parse(readFileSync(resolve(issuesPath), "utf8")) : [];
