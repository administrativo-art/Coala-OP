...
test('data de admissão só pode mudar na etapa do contador', () => {
  assert.equal(canUpdateExpectedAdmissionDate({ status: 'collecting_documents', currentStage: 'documents' }), false);
  assert.equal(canUpdateExpectedAdmissionDate({ status: 'reviewing_documents', currentStage: 'document_review' }), false);
...
test('data de admissão exige uma data real no formato ISO sem horário', () => {
