...
test("preserva linhas da stack e não interpreta Error como protocolo de URL", () => {
  ...
  error.stack = "Error: fixture com Bearer secret-token e person@example.com CPF 12345678900\n    at run (/Users/person/project/job.ts:10:2)\n    at next (/app/worker.ts:20:4)";
  const sanitized = sanitizeError(error);
  assert.equal(sanitized.stack?.includes("secret-token"), false);
  assert.equal(sanitized.stack?.includes("person@example.com"), false);
  assert.equal(sanitized.stack?.includes("12345678900"), false);
  assert.match(sanitized.stack ?? "", /^Error: fixture com \[REDACTED\]/);
  assert.match(sanitized.stack ?? "", /\n    at run \(/);
  assert.match(sanitized.stack ?? "", /\n    at next \(/);
  assert.equal((sanitized.stack?.match(/\n/g) ?? []).length, 2);
});

test("limita stack sem linearizar os frames preservados", () => {
  const stack = ["Error: fixture", ...Array.from({ length: 500 }, (_, index) => `    at function${index} (/app/file${index}.ts:${index + 1}:2)`)].join("\n");
  const sanitized = sanitizeStack(stack);
  assert.ok(sanitized);
  assert.equal((sanitized?.length ?? 0) <= SANITIZE_LIMITS.MAX_STACK_LENGTH, true);
  assert.match(sanitized ?? "", /^Error: fixture\n    at function0/);
...
test("limita objetos cíclicos, arrays e payloads gigantes", () => {
  ...
  const result = sanitizeMetadata({
    route: "/api/example",
    technicalContext: circular,
    values: Array.from({ length: 50 }, (_, index) => index),
    reasonCode: "x".repeat(5_000),
  }, { allowedMetadataKeys: ["technicalContext", "values"] });
  assert.deepEqual((result.values as unknown[]).length, 20);
  assert.equal((result.technicalContext as Record<string, unknown>).self, "[CIRCULAR]");
