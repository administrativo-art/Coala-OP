...
test("reporter aceita fábricas injetadas para eventId e timestamp sem tornar o teste frágil", () => {
  const sink = new CaptureSink();
  ...
  assert.equal(reference.eventId, "00000000-0000-4000-8000-000000000001");
  assert.equal(sink.events[0]?.occurredAt, "2026-08-27T12:00:00.000Z");
});
...
test("sink lento, fingerprint inválido, sanitizador hostil e release desconhecida não bloqueiam o fluxo", () => {
  ...
  const sink = new CaptureSink();
  assert.doesNotThrow(() => reportSystemError({
    error: new Error("original metadata"),
    source: "test",
    metadata: hostile,
    release: "unknown",
    sink,
...
test("erro inesperado retorna eventId sem stack nem error.message bruto", async () => {
  ...
  assert.equal(response.headers.get("x-correlation-id"), "flow-2");
  assert.match(payload.error.eventId, /^[0-9a-f-]{36}$/);
  assert.equal(payload.error.requestId, "request-2");
  assert.equal(payload.error.correlationId, "flow-2");
...
test("wrapper preserva sucesso e propaga requestId e correlationId", async () => {
  ...
  const response = await handler(new NextRequest("https://example.test/api/example", {
    headers: { "x-request-id": "upstream-request", "x-correlation-id": "flow-7" },
  }), { params: Promise.resolve({}) });
  assert.equal(response.status, 201);
  assert.equal(response.headers.get("x-request-id"), "upstream-request");
  assert.equal(response.headers.get("x-correlation-id"), "flow-7");
