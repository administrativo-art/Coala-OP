  ...
  try {
    ...
    if (process.argv.includes("--write-baseline")) {
      ...
      if (violations.length > 0) {
        process.stderr.write(`Contrato de erro violado em ${violations.length} entrada(s):\n`);
        for (const violation of violations) {
          process.stderr.write(`- ${violation.key}: atual=${violation.current}, baseline=${violation.allowed}\n`);
        }
        process.exitCode = 1;
      } else {
    ...
    process.stderr.write(`Falha ao verificar o contrato de erro: ${error instanceof Error ? error.message : String(error)}\n`);
    process.exitCode = 1;
  }
}
