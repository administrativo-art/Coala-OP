...
async function loadConsumptionReportEntries(years: number[]): Promise<ConsumptionEntry[]> {
...
async function loadProductToBaseProductMap(): Promise<Map<string, string>> {
...
/**
 * Recalcula `stockLevels.<kioskId>.min` de cada insumo (baseProduct) todo dia 1 do mês,
 * usando a média de consumo dos últimos 6 meses + 30% de margem de segurança.
 *
 ...
 *
 * `stockLevels.<kioskId>.override === true` trava o item nesse quiosque — o job pula e não
...
export const recalculateMinimumStock = onSchedule(
  ...
  async () => {
    ...
    const [productToBaseProduct, baseProductsSnap] = await Promise.all([
      loadProductToBaseProductMap(),
      db.collection("baseProducts").get(),
    ]);

    const [reportEntries, movementEntries] = await Promise.all([
      loadConsumptionReportEntries(years),
      loadMovementHistoryEntries(cutoff, productToBaseProduct),
    ]);
    ...
    console.log(
      `[recalculateMinimumStock] ${updated} estoques mínimos atualizados; ${skippedArchived} insumos arquivados ignorados; ${skippedOverride} pulados por override; ${skippedNoData} pulados por falta de dado de consumo.`,
    );
  },
);
